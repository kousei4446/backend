...
INSERT INTO quizzes (
  ...
  ('easy', 11, 'Pythonでリストの末尾の要素を取り除くメソッドは？', 'remove()', 'pop()', 'delete()', 'cut()', 'B', '`pop()`で末尾の要素を取り除きます。'),
  ('easy', 12, 'Pythonで文字列をすべて大文字に変換するには？', 'upper()', 'capitalize()', 'toupper()', 'uppercase()', 'A', '`upper()` で全て大文字に変換できます。'),
  ('easy', 13, 'Pythonで1から10までの範囲を生成するには？', 'range(1,10)', 'range(1,11)', 'range(10)', 'range(0,10)', 'B', '`range(1,11)` は1〜10を生成します。'),
  ('easy', 14, 'Pythonで要素の数をカウントするリストメソッドは？', 'length()', 'count()', 'size()', 'number()', 'B', '`count()` で特定要素の出現数をカウントします。'),
  ('easy', 15, 'Pythonのリストを昇順に並べ替えるには？', 'sort()', 'order()', 'arrange()', 'ascend()', 'A', '`sort()`で昇順に並べ替えられます。'),
  ('easy', 16, 'Pythonで1行コメントを書くには？', '#', '//', '--', '/* */', 'A', '`#` を使います。'),
  ...
  ('easy', 24, 'Pythonで空のリストを作成するには？', '[]', '{}', '()', 'empty()', 'A', '`[]` は空のリストです。'),
  ('easy', 25, 'Pythonで文字列の長さを取得するには？', 'size()', 'length()', 'len()', 'count()', 'C', '`len()` を使います。'),
  ('easy', 26, 'Pythonで`True`と`False`は？', '整数', '文字列', 'ブール型', 'リスト', 'C', 'Pythonの真偽値はブール型です。'),
  ...
  ('normal', 11, 'Pythonで文字列の先頭や末尾の空白を削除するメソッドは？', 'strip()', 'trim()', 'remove()', 'clean()', 'A', '`strip()` で前後の空白が削除されます。'),
  ('normal', 12, 'Pythonのタプルはどのような特徴を持つ？', '可変である', '順序がない', '変更不可である', '辞書のキーに使えない', 'C', 'タプルはイミュータブル（変更不可）です。'),
  ('normal', 13, 'Pythonでリストをコピーする正しい方法は？', 'copy()', 'list()', '[:]でスライス', 'すべて正しい', 'D', 'どれも有効なコピー方法です。'),
  ('normal', 14, 'Pythonで例外の種類を指定するには？', 'except IOError:', 'except = IOError:', 'catch IOError:', 'try IOError:', 'A', '`except IOError:` のように書きます。'),
  ('normal', 15, 'Pythonで辞書の値だけ取得するメソッドは？', 'values()', 'keys()', 'items()', 'get()', 'A', '`values()` で値だけ取得できます。'),
  ('normal', 16, 'Pythonのスライス記法 [1:4] の意味は？', '1から4まで', '0から4まで', '1から3まで', '2から5まで', 'C', '終了インデックスは含まれません（1〜3）。'),
  ('normal', 17, 'Pythonで例外を無視するには？', 'try: except: pass', 'ignore: error', 'skip error', 'raise ignore', 'A', '`pass` を使って何もしないようにします。'),
  ('normal', 18, 'Pythonの`enumerate()`関数は何を返す？', 'キーと値', 'インデックスと要素', '文字列とリスト', 'リストと辞書', 'B', '`enumerate()`はインデックスと要素のタプルを返します。'),
  ('normal', 19, 'Pythonで複数の例外をまとめて処理するには？', 'except (A, B):', 'except A + B:', 'try A, B:', 'handle A, B:', 'A', 'タプルでまとめて書けます。'),
  ('normal', 20, 'Pythonでリストから要素を削除する方法は？', 'remove()', 'delete()', 'discard()', 'popitem()', 'A', '`remove()` は指定した要素を削除します。'),
  ('normal', 21, 'Pythonで文字列を数値に変換するには？', 'int()', 'str()', 'ord()', 'convert()', 'A', '`int()` を使って文字列を数値化します。'),
  ('normal', 22, 'Pythonの関数内で変数をグローバルに扱うには？', 'global', 'extern', 'public', 'static', 'A', '`global` を使います。'),
  ('normal', 23, 'Pythonで辞書のキーが存在するか確認するには？', `'key' in dict`, `'dict'.hasKey('key')`, `'dict'.contains('key')`, `'dict'.keyExists('key')`, 'A', '`in` で確認できます。'),
  ('normal', 24, 'Pythonでデフォルト辞書（キーがなくても初期値を返す）に使うのは？', 'defaultdict', 'dictset', 'autodict', 'lazydict', 'A', '`collections.defaultdict` を使います。'),
  ('normal', 25, 'Pythonでラムダ関数を使う理由は？', '簡潔に書ける', '高速化のため', 'エラーを回避するため', 'メモリ削減のため', 'A', '短い関数を簡潔に記述できます。'),
  ('normal', 26, 'Pythonで複数のリストを同時にループ処理する関数は？', 'zip()', 'join()', 'map()', 'reduce()', 'A', '`zip()` でまとめてループ可能です。'),
